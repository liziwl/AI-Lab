...
def readData(filename):
    ...
    # 8行开始，数据本体
    f = open(filename, 'r')
    Pattern = ["NAME : (.*)", "VERTICES : ([0-9]*)", "DEPOT : ([0-9]*)", "REQUIRED EDGES : ([0-9]*)","NON-REQUIRED EDGES : ([0-9]*)",
               "VEHICLES : ([0-9]*)", "CAPACITY : ([0-9]*)", "TOTAL COST OF REQUIRED EDGES : ([0-9]*)"]
    data = f.readlines()
    count = 0
    fData =[]
    for it in data:
...
if __name__ == '__main__':
    ...
    print "-------------\n"
    dijkstra = dijkstra.Dijkstra(vmap, 56, 36)
